...
7. v3: если автономный агент завершает ход **без текстового ответа**, вместо пустой
   карточки родитель получает ping («завершился без финального ответа») — возобновите
   через `resume_agent` с просьбой дать отчёт; карточка done без summary также
   помечена явно (noSummary + подсказка resume_agent).
...
помечает `stalled` + уведомление. Пока stall держится — повторные уведомления каждые
`stallRepingTicks` тиков (дефолт 30; 0 — только первое). Тихий «waiting» (ход завершён,
